...
class SocketServer {
  ...
  configurePing() {
    ...
    const clientMs = Number(process.env.SOCKET_PING_INTERVAL);
    const clientInterval = Number.isFinite(clientMs) && clientMs > 0 ? clientMs : 10000;

    const serverMs = Number(process.env.SERVER_SOCKET_PING_INTERVAL);
    const serverInterval = Number.isFinite(serverMs) && serverMs > 0 ? serverMs : 2000;

    // Ping real clients (load balancer connections use isServer and are handled below)
    ...
    // Ping load-balancer / server-authenticated connections
...
class ClientSocket extends Socket {
  ...
  heartbeat() {
    ...
    const serverMs = Number(process.env.SERVER_SOCKET_PING_INTERVAL);
    const interval = Number.isFinite(serverMs) && serverMs > 0 ? serverMs : 2000;
